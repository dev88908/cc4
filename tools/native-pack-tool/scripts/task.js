...
function getParams() {
    ...
    const raw = fs.readJsonSync(ps.join(getCurrentProject(), CocosConfig));
    if (!raw) {
        throw new Error(`can not get cocosParams in ${CocosConfig}`);
    }
    // Support nested cocosParams (Creator build output) or flat config
    currentInfo.cocosParams = raw.cocosParams != null ? raw.cocosParams : raw;
    return currentInfo.cocosParams;
}
