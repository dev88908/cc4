...
class NativePackTool {
    ...
    get versionParser() {
        if (!this._versionParser) {
            const scriptPathNative = ps.join(utils_1.Paths.enginePath, 'native/cmake/scripts/plugin_support/plugin_cfg.js');
            const scriptPathRoot = ps.join(utils_1.Paths.enginePath, 'cmake/scripts/plugin_support/plugin_cfg.js');
            const scriptPath = fs.existsSync(scriptPathNative) ? scriptPathNative : scriptPathRoot;
