...
export abstract class NativePackTool {
    ...
    private get versionParser() {
        if (!this._versionParser) {
            const scriptPathNative = ps.join(Paths.enginePath, 'native/cmake/scripts/plugin_support/plugin_cfg.js');
            const scriptPathRoot = ps.join(Paths.enginePath, 'cmake/scripts/plugin_support/plugin_cfg.js');
