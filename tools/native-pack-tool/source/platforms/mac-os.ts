...
export abstract class MacOSPackTool extends NativePackTool {
    ...
    async xcodeFixAssetsReferences() {
        const nativePrjDir = this.paths.nativePrjDir;
        const xcodePath = ps.join(this.params.enginePath, 'tools/native-pack-tool/xcode');
        const xcodePathLegacy = ps.join(this.params.enginePath, 'scripts/native-pack-tool/xcode');
        const xcode = require(fs.existsSync(ps.join(xcodePath, 'package.json')) ? xcodePath : xcodePathLegacy);
